...
using DNU.Usuarios.APIService.App_Start.Resolvers;
        ...
        public static string obtenerCadenaAzure(string key)
        {
            ...
            try
            {
                ...
                if (activarAzure)
                {
                    ...
                    {
                     //   new Logueo("").Evento("Valor Azure obtenido correctamente del secreto:" + clave);
                        Respuesta = respuestaObtenerCadena.valorAzure;
                    }
                    else
                    {
                        new Logueo("").Error("Error al consultar Secreto Azure: " + clave + " Json:" + JsonConvert.SerializeObject(respuestaObtenerCadena));
                    }
                }
                else
                {
                    new Logueo("").Error("Esta desactivada la consulta de Azure");
                }
                return Respuesta;
            }
            catch (Exception ex)
